...
export const PaymentPage=()=>{
    ...
    useEffect(()=>{
       const fetchFees=async()=>{
        if(authState && authState.isAuthenticated){
            const url=`http://localhost:8081/payments/search/findByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
    ...
    async function checkout() {
        ...
        const url=`http://localhost:8081/payment/secure/payment-intent`;
        const requestOptions={
            method:'POST',
            headers:{
        ...
        stripe.confirmCardPayment(
            ...
            if(result.error){
                ...
                const url=`http://localhost:8081/payment/secure/payment-complete`;
                const requestOptions={
                    method:'PUT',
                    headers:{
                        Authorization:`Bearer ${authState?.accessToken?.accessToken}`,
        ...
        <div className="container">
            {fees>0 && <div className="card mt-3">
                ...
                <div className="card-body">
                    ...
                    <button disabled={submitDisabled} type='button' className="btn btn-md main-color text-white mt-3" onClick={checkout}>Pay fees</button>
                </div>
            </div>}
            {fees===0 && <div className="mt-3">
                <h5>You have no fees!</h5>
                <Link type='button' className='btn main-color text-white' to='/search'>Explore top arts</Link>
            </div>}
